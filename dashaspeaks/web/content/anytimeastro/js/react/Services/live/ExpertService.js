        ...
        return fetch(this.config.EXPERT_LIST_URL_2, {
            ...
            body: JSON.stringify(data)
                ...
                if (result != null && result != undefined) {
                    var date = new Date();
                    let timeOffesetClientServer = Math.ceil((result.Result.Config.CurrentTime - date.getTime()) / 1000);
                    date.setTime(date.getTime() + (1440 * 60 * 1000));
                    //console.log("ServerTimestamp-", result.Result.Config.CurrentTime, "LocalTimestamp-", date.getTime());
                    var expires = "; expires=" + date.toGMTString();
                    document.cookie = "ClientTimeOffset=" + timeOffesetClientServer + expires + "; path=/";
                }
                    return result;
                }).catch(error => {
                    this.handleResponseError(401);
