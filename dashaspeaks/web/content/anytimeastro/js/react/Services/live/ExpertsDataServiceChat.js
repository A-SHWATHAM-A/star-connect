var isDiscountOfferFiltertoApply = false;
class ExpertsDataServiceChat {
    ...
    getShuffledLPORankList(dataitems) {
        //debugger;
        dataitems = dataitems.sort((firstItem, secondItem) => firstItem.LpoRank - secondItem.LpoRank);
        return dataitems;
    }
    ...
    GetGroupData(dataitems, GroupObject, showGlobalOffer, freebatchcount =0) {
        //console.log("dataitems ",dataitems);
        var ModuleOfferType = 0;
        let filteredGroupData = {};
        let localfilterdata = {};
        let isHappyHourGroup = false;
        let ApplicableOffer = {
            NONE:0,
            FREE: 1,
            LOW_PRICE: 2,
        ...
        if (checkIfOfferAvailable(eEligibleArea.CHAT)) {
            ...
            } else if ((ModuleOfferType == 5 || ModuleOfferType == 6 || ModuleOfferType == 8 || ModuleOfferType == 9 || ModuleOfferType == 13) && ModuleOfferId > 0) {
                MyCurrentApplicableOffer = ApplicableOffer.FREE;
            }
            else if ((ModuleOfferType == 14) && ModuleOfferId > 0) {
                MyCurrentApplicableOffer = ApplicableOffer.DISCOUNT;
            }
        }
        ...
        let doeFlag = GroupObject.hasOwnProperty('doe') ? GroupObject.doe : -1;
        ...
        else if (hlpFlag == 1 && MyCurrentApplicableOffer == ApplicableOffer.DISCOUNT) {
            fmeFlag = -1;
            hlpFlag = -1;
        }
        if (hlpFlag != -1) {
        ...
        if (doeFlag != -1 && isDiscountOfferFiltertoApply) {
            //means 1 or 0
            filteredGroupData = filteredGroupData.filter(e => { return e.doe == doeFlag });
        }


        if (filteredGroupData != null && (hlpFlag != -1 || fmeFlag != -1 || doeFlag!=-1)) {
            if (fmeFlag != -1) {
            ...
            if (filteredGroupData != null) {
                if (MyCurrentApplicableOffer == ApplicableOffer.LOW_PRICE || MyCurrentApplicableOffer == ApplicableOffer.DISCOUNT) {
        ...
        if (filteredGroupData != null) {
            ...
            //MARK: 2 Filter HappyHourExpert
            if (GroupObject.hasOwnProperty('hhe')) {
                let hheFlag = GroupObject.hasOwnProperty('hhe') ? GroupObject.hhe : -1;
                if (hheFlag != -1) {
                    isHappyHourGroup = true;
                    filteredGroupData = filteredGroupData.filter(e => { return e.isHappyHourEnabled == hheFlag });
                }
            }
            ...
            if (isHappyHourGroup) {
                ...
                if (GroupObject.hasOwnProperty('mip') && GroupObject.mip && GroupObject.mip > 0) {
                    filteredGroupData = filteredGroupData.filter(e => { return e.HappyHourRate >= GroupObject.mip });
                }
                if (GroupObject.hasOwnProperty('mxp') && GroupObject.mxp && GroupObject.mxp > 0) {
                    filteredGroupData = filteredGroupData.filter(e => { return e.HappyHourRate <= GroupObject.mxp });
                }
            } else {
                //MARK: 4 Filter minPrice
                if (GroupObject.hasOwnProperty('mip') && GroupObject.mip && GroupObject.mip > 0) {
            ...
            if (fmeFlag == 1 && showExpertinFree == 1 && MyCurrentApplicableOffer == ApplicableOffer.FREE) {
                ...
                if (GroupObject.hasOwnProperty('frnk') && GroupObject.frnk && GroupObject.frnk > 0) {
                    ...
                    freebatchcountFlag = true;
                }
            } else if (hlpFlag == 1 && MyCurrentApplicableOffer == ApplicableOffer.LOW_PRICE) {
                //MARK: 8 Filter Priority
                if (GroupObject.hasOwnProperty('lprnk') && GroupObject.frnk && GroupObject.lprnk > 0) {
                    filteredGroupData = this.getShuffledLPORankList(filteredGroupData);
                    freebatchcountFlag = true;
                }
            } else {
                //MARK: 8 Filter Priority
            ...
            if (freebatchcountFlag) {
                if (filteredGroupData.length > 0 && freebatchcount > 0) {
                    if (filteredGroupData.length > freebatchcount) {
                        filteredGroupData = filteredGroupData.slice(0, freebatchcount);
                    }
                    filteredGroupData = filteredGroupData.sort(() => Math.random() - 0.5);
                }
            }

            //MARK: 9 sorting
            if (MyCurrentApplicableOffer == ApplicableOffer.FREE && showExpertinFree) {
                ...
                filteredGroupData = filteredGroupData.filter(e => {
                    let timestamp = e.NextFreeSession.match(/\d+/)[0];
                    let dateddd = new Date(parseInt(timestamp));
                    let dateddx = new Date(parseInt(ServerCurrentTimeStampForList));
                    traceLog(dateddd.toISOString() + "----" + dateddx.toISOString());  // Outputs the datetime in ISO 8601 format
                    return (timestamp <= ServerCurrentTimeStampForList || e.NextFreeSession == "" || e.NextFreeSession == null)
                });
            }
            ...
            if (filteredGroupData != null) {
                ...
                if (totFlag > 0) {
                    ...
                    if (MyCurrentApplicableOffer == ApplicableOffer.LOW_PRICE && hlpFlag==1) {
                    ...
                    } else if (MyCurrentApplicableOffer == ApplicableOffer.DISCOUNT && doeFlag == 1) {
                        //mark low price expert and get total out of it
                        localfilterdata = filteredGroupData.map((obj) => { (obj.doe == 1) ? obj.showinfree = 1 : obj.showinfree = 0; return obj; });
                        filteredGroupData = localfilterdata;
                    }
                        ...
                        // if total experts is greater than equals to _MinimumFreeExpertsCount, than mark free, else no free
                        if (ModuleOfferType == 13) {
                            if (filteredGroupData.length >= _MinimumFreeExpertsCount) {
                                ...
                                filteredGroupData = localfilterdata;
                                traceLog("free counter", filteredGroupData.length);
                                if (filteredGroupData.length >= _MinimumFreeExpertsCount) {
                                    isRequiredExpertsAvailable = true;
                                    _gFreeExpertCounter = filteredGroupData.length;
                                }
                            }
    ...
    shufflegroupwiseData(shuffledOnlineAstrologers, groupname, settingsjson, showGlobalOffer) {
       // console.log("shuffledOnlineAstrologers", shuffledOnlineAstrologers);
        ...
        let freebatchcount = 0;
        let groupArray = [];

        if (settingsjson.settings.hasOwnProperty("freebatchcount") && settingsjson.settings.freebatchcount != undefined && parseInt(settingsjson.settings.freebatchcount) > 0) {
            freebatchcount = settingsjson.settings.freebatchcount;
        }
        if (groupname == "groups") {
        ...
        if (groupArray instanceof Array && groupArray.length > 0) {
            ...
            for (let i = 0; i < groupArray.length; i++) {
                ...
                if (shuffledOnlineAstrologers.length > 0) {
                    ...
                    let _groupResult = this.GetGroupData(shuffledOnlineAstrologers, groupArray[i], showGlobalOffer, freebatchcount);
    ...
    getFilteredHappyHourData(happyHourExpertsList, currentUTCTimeStamp) {
        let HappyHourListCurrentTimestamp = happyHourExpertsList.filter(e => {
            return currentUTCTimeStamp >= e.HHSlot.match(/\d+/)[0] && currentUTCTimeStamp <= e.HappyHourEndTime.match(/\d+/)[0]
        });

        

        HappyHourListCurrentTimestamp = HappyHourListCurrentTimestamp.map((obj) => {


            let happyhourRate = obj.ChargeRate;
            if (obj.ActualChargeRate > 0) {
                happyhourRate = obj.ActualChargeRate;
            }

            let tempexpertChargeToShow = parseFloat(obj.HHDisPer) / 100 * parseFloat(happyhourRate);
            let updatedhappyhourprice = happyhourRate - tempexpertChargeToShow;
            if (updatedhappyhourprice < obj.ChargeRate) {
               
               let HappyHourExpertCharge = parseFloat(updatedhappyhourprice);
 
                if (gIsIndia == "True") {
                    HappyHourExpertCharge = Math.ceil(HappyHourExpertCharge);
                } else {
                    HappyHourExpertCharge = parseFloat(HappyHourExpertCharge).toFixed(2);
                }
                obj.HappyHourRate = HappyHourExpertCharge;

                let OfferPercent = (((obj.BaseRate - HappyHourExpertCharge) / obj.BaseRate) * 100);

                OfferPercent = Math.floor(OfferPercent);
                obj.HappyHourPercent = OfferPercent;
                obj.isHappyHourEnabled = 1;
            }
        ...
        return HappyHourListCurrentTimestamp;
    }


    getFilteredDiscountOfferData(expertsdataobj) {

        var expertsDataObjNew = expertsdataobj;

        if (typeof _CurrentOfferObj === 'object' && _CurrentOfferObj != null) {

            if (parseInt(_CurrentOfferObj.OfferType) == 14) {

                let isofferForLPOExpertsOnly = _CurrentOfferObj.IsForLPOExpOnly;
                let minExpertChargeRateRq = _CurrentOfferObj.MinExpRate;
                //if discount offer applicable for lowprice experts only
                if (isofferForLPOExpertsOnly) {
                    expertsDataObjNew = expertsDataObjNew.map(item => ({
                        ...item,
                        doe: (isofferForLPOExpertsOnly && item.IsLowPrice === 1 && item.ChargeRate > minExpertChargeRateRq) ? 1 : 0
                    }));

                } else { //if discount offer applicable for all experts
                    expertsDataObjNew = expertsDataObjNew.map(item => ({
                        ...item,
                        doe: (item.ChargeRate > minExpertChargeRateRq) ? 1 : 0
                    }));
        ...
        return expertsDataObjNew;
    }


    getFilteredAstrologers(dataitems, showFree, AIapplyFilter = false, _currentUTCTimeStampForList) {
           ...
           offline*/
            ...
            if (settingsjson != null && typeof settingsjson === "object" && settingsjson.hasOwnProperty("settings")) {
                ...
                if ((settingsjson.hasOwnProperty("groups") && settingsjson.groups.length > 0) || (settingsjson.hasOwnProperty("timewise") && settingsjson.timewise.hasOwnProperty("tgroups") && settingsjson.timewise.tgroups.length > 0) || settingsjson != null && typeof settingsjson === "object" && settingsjson.hasOwnProperty("rastro")) {
                    ...
                    // filter Happy Hour Enabled astrologers
                    let filteredHappyHourData = OldAstrologerItem.filter(e => { return e.HappyHourEndTime != null });

                    let HappyHourEnabledData = this.getFilteredHappyHourData(filteredHappyHourData, _currentUTCTimeStampForList);


                    // filter Happy Hour not Enabled astrologers
                    let RemainingOldAstrologerItem = OldAstrologerItem.filter(function (item) {
                        return HappyHourEnabledData.filter(function (item2) {
                            return item.PsychicId == item2.PsychicId;
                        }).length == 0;
                    });


                    //merge both lists again in OldAstrologerItem
                    OldAstrologerItem = HappyHourEnabledData;
                    Array.prototype.push.apply(OldAstrologerItem, RemainingOldAstrologerItem);


                    //apply doe key for discount offer
                    //settingsjson.disofrWithfilter = 1;
                    if (settingsjson.settings.hasOwnProperty("disofrWithfilter") && settingsjson.settings.disofrWithfilter > 0) {
                        isDiscountOfferFiltertoApply = true;
                    }

                    let DiscountOfferEnabledData = this.getFilteredDiscountOfferData(OldAstrologerItem);
                    OldAstrologerItem = DiscountOfferEnabledData;
                    ...
                    if (_RecommendedListFilter === true && (settingsjson.rastro != null && typeof settingsjson.rastro === "object" && settingsjson.rastro.hasOwnProperty("rgroups") && settingsjson.rastro.rgroups != null && typeof settingsjson.rastro.rgroups === "object" && settingsjson.rastro.rgroups.length > 0)) {
                        ...
                        let freebatchcount = 0;
                        if (settingsjson.hasOwnProperty("freebatchcount") && settingsjson.freebatchcount != undefined && parseInt(settingsjson.freebatchcount) > 0) {
                            freeBatchCount = settingsjson.freebatchcount;
                        }

                        for (let i = 0; i < settingsjson.rastro.rgroups.length; i++) {
                            let shuffledGroupsData = this.GetGroupData(recommendedAstrologersList, settingsjson.rastro.rgroups[i], showGlobalOffer, freebatchcount);
    ...
    /** recommended low price astrologers list */
    async getLowPriceExpertList(_showmore, searchText, filterEnum, isFav, catid, pageoptions = null, AIapplyFilter = false, TotalExpertsCounttoFetch=0) {
        //debugger;
        let _RecommendedListFilter = false;
        ...
        var ExpertSearchOptions = {
            isActive: true,
            page: 1,
            pageSize: TotalExpertsCounttoFetch,
        ...
        if (this.state.isLoaded) {
            if (this.state.paging == null) {
                this.state.paging = { page: 0, pageSize: TotalExpertsCounttoFetch, totalPages: 0 };
            } else {
    ...
    async getExpertList(_showmore, searchText, filterEnum, isFav, catid,pageoptions=null,AIapplyFilter=false) {
        ...
        if (checkIfOfferAvailable(eEligibleArea.CHAT)) {
            ...
            if (localModuleOfferType > 0 && (localModuleOfferType == 5 || localModuleOfferType == 6 || localModuleOfferType == 8 || localModuleOfferType == 9 || localModuleOfferType == 13 || localModuleOfferType == 12)) {
               
                ExpertSearchOptions.orderBy = 11;
                ExpertSearchOptions.IsShowfreeExpert = true;
            }
            else if (localModuleOfferType > 0 && (localModuleOfferType == 10)) {
                ExpertSearchOptions.orderBy = 12;
            }
            else if (localModuleOfferType > 0 && (localModuleOfferType == 14)) {
                if (isofferForLPOExpertsOnly) {
                    ExpertSearchOptions.orderBy = 12;
                }
            }
        ...
        if ((typeof searchText === 'string' && searchText != "") || (typeof this.state.SearchExpert === 'string' && this.state.SearchExpert != "")) {
            _SearchExpert = true;
            searchText = (searchText == "") ? this.state.SearchExpert : searchText;
            ExpertSearchOptions.orderBy = 1;
            ExpertSearchOptions.IsShowfreeExpert=false;
        }
        else {
            _SearchExpert = false;
        }
        ...
        if (ExpertListService) {
            ...
            if (ExpertListService.StatusCode == 200) {
                ...
                if (serviceConfig.CurrentTime > 0) {
                    ...
                    ServerCurrentTimeStampForList = serviceConfig.CurrentTime;
                ...
                if (_SearchExpert) {
                    ...
                    olddataitems = this.getFilteredAstrologers(dataitems, false, _RecommendedListFilter, parseInt(ServerCurrentTimeStampForList));
                    //olddataitems = dataitems;                    
                }
                else {
                    Array.prototype.push.apply(olddataitems, dataitems);
                    //do not shuffle/ do not show free - for search filter
                    olddataitems = this.getFilteredAstrologers(olddataitems, checkIfOfferAvailable(eEligibleArea.CHAT), _RecommendedListFilter, parseInt(ServerCurrentTimeStampForList));
                }
    ...
    noRecordFound() {
        return '<div class="col-md-12 pt-5 pb-5 text-center bg-white" ><img src="' + _atacommonCdnPrefix+'dashaspeaks/web/content/images/no-favorites.svg" class = "mt-4" alt = "Content Not Found" /><h4 class="weight500 font20 colorblack mt-4 mb-2">' + getLanguageKeyString("CWA_No_Expert_Found") + '</h4></div>';
    }
            ...
            case "chat-online":
               // let jsonobj = { "PsychicId": expert.PsychicId, "DisplayName": expert.DisplayName, "freechatminute": expert.freechatminute, "expertDetailsUrl": '', "expertChatCharge": expertCharge, "expertUSDChatCharge": expertUSDCharge, "expertChargeType": expertChargeType, "currency": currency, "ReviewCount": expert.ReviewCount, "Rating": expert.Rating, "isFree": isFreeChat };
                let jsonobj = {"PsychicId": expert.PsychicId, "expertFormatCharge": currency + expertCharge, "categoryName": categoryName, "expertPic": expertPic, "onlineStatus": expert.onlineStatus, "DisplayName": expert.DisplayName,
                    ...
                    "expertChargeType": expertChargeType, "currency": currency, "ReviewCount": expert.ReviewCount, "Rating": expert.Rating, "isFree": isFreeChat, "IsChargeOverride": config.IsChargeOverride, "ModuleOfferType": config.ModuleOfferType, "NextTimeCount": config.NextTimeCount, "NextSessionCounter": config.NextSessionCounter, "moduleofferId": config.moduleofferId, "LowPriceOfferMin": config.LowPriceOfferMin, "IsAIExpert": expert.IsAIExpert, "IsHappyHour": config.IsHappyHour, "HappyHourEndTime": config.HappyHourEndTime, "updatedDiscountedprice": config.updatedDiscountedprice
                }
    ...
    renderFreeConsultationInfoBadge(offerUrl, currentLanguage) {
        let expertprofilepic = _atacommonCdnPrefix+"dashaspeaks/web/content/images/free-consultation-badge.png";
    ...
    getFreeConsulationCard(expertconfig, currentLanguage) {
        ...
        let expertprofilepic = _atacommonCdnPrefix+"dashaspeaks/web/content/images/free-consultation-astro.png";
        let culturelangcode = getUiCultureCode();
        ...
        return '<div id="ATAAIOfferTile" class="psychic-card expertOnline ask-guruji ' + AIOffferClass +'" ><div class="psychic-card-inner overflow-hidden position-relative">' + itemsection1 + itemsection2 +'</div></div>';
    }
    
    /** recommended low price astrologer card */
    getLowPriceRecommendedPsychicCard(expertdata, expertconfig, aiAssistentPlnId, NextOfferObj) {
        ...
        if (typeof expertdata.Picture != "undefined" && expertdata.Picture != null && expertdata.Picture != "") {
            ...
            expertprofilepic = _atacommonCdnPrefix + "dashaspeaks/psychics/" + expertdata.Picture + "";
        }
        ...
        let walletUrl = gWebsitePrefix + culturelangcode + "wallet/" + url_string.replace(/&amp;/g, '&');
        ...
        let AIWizardsessionDataobj = {};
        requiredBalance = expertCharge * 5;

        if (NextOfferObj != null) {
            localModuleOfferType = NextOfferObj.OfferType;
            if (NextOfferObj.OfferType > 0 && (NextOfferObj.OfferType == 10)) {
                lowPriceOffferMin = NextOfferObj.OfferMinutes;
                let _discountedOfferPrice = NextOfferObj.LowPriceRate;
                requiredBalance = NextOfferObj.LowPriceRate * NextOfferObj.OfferMinutes;
                if (requiredBalance <= 0) {
                    requiredBalance = 50;
                }
                IsChargeOverride = false;
                if (expertdata.IsLowPrice > 0 && _discountedOfferPrice > 0) {
                    //discountedOfferPrice = 5;
                    if (gloggedInUserid > 0) {
                        if (parseInt(NextOfferObj.OfferId) > 0) {
        ...
        if (IsFreeChat || islowpriceoffer) {
            ...
            if (walletRechargeUrl.indexOf("?") >= 0) {
                if (walletRechargeUrl.lastIndexOf("?") == walletRechargeUrl.length - 1) {
                    walletRechargeUrl += "fcm=true";
                } else if (walletRechargeUrl.indexOf("fcm") == -1) {
                    walletRechargeUrl += "&fcm=true";
                }
            } else {
                walletRechargeUrl += "?fcm=true";
            ...
            if (walletUrl.indexOf("?") >= 0) {
                if (walletUrl.lastIndexOf("?") == walletUrl.length - 1) {
                    walletUrl += "fcm=true";
                } else if (walletUrl.indexOf("fcm") == -1) {
                    walletUrl += "&fcm=true";
                }
            } else {
                walletUrl += "?fcm=true";
        ...
        if (gwalletBalance >= requiredBalance) {
            if (expertUrl.indexOf("?") >= 0) {
                if (expertUrl.lastIndexOf("?") == expertUrl.length - 1) {
                    expertUrl += "opaction=chat&eid=" + expertdata.PsychicId;
                } else {
                    if (expertUrl.indexOf("opaction") == -1) {
                        expertUrl += "&opaction=chat";
                    }
                  
                    if (expertUrl.indexOf("eid") == -1) {
                        expertUrl += "&eid=" + expertdata.PsychicId;
                    }
                }
            } else {
                expertUrl += "?opaction=chat&eid=" + expertdata.PsychicId;
            }
            walletRechargeUrl = expertUrl; // assign as, walletRechargeUrl is already in used 
        } else {
            if (NextOfferObj != null && NextOfferObj.OfferType == 10) {
                if (walletRechargeUrl.indexOf("?") >= 0) {
                    if (walletRechargeUrl.lastIndexOf("?") == walletRechargeUrl.length - 1) {
                        walletRechargeUrl += "opaction=chat&eid=" + expertdata.PsychicId;
                        walletRechargeUrl += "&selplnid=" + aiAssistentPlnId;
                    } else {
                        if (walletRechargeUrl.indexOf("opaction") == -1) {
                            walletRechargeUrl += "&opaction=chat&selplnid=" + aiAssistentPlnId;
                        }
                        if (walletRechargeUrl.indexOf("selplnid") == -1) {
                            walletRechargeUrl += "&selplnid=" + aiAssistentPlnId;
                        }
            ...
            if (walletUrl.indexOf("?") >= 0) {
                if (walletUrl.lastIndexOf("?") == walletUrl.length - 1) {
                    walletUrl += "opaction=chat&eid=" + expertdata.PsychicId;

                } else {
                    if (walletUrl.indexOf("opaction") == -1) {
                        walletUrl += "&opaction=chat";
                    }

                    if (walletUrl.indexOf("eid") == -1) {
                        walletUrl += "&eid=" + expertdata.PsychicId;
                    }
                }
            } else {
                walletUrl += "?opaction=chat&eid=" + expertdata.PsychicId;
            ...
            AIWizardsessionDataobj = {
                "PsychicId": expertdata.PsychicId,
                "expertFormatCharge": expertdata.ChargeRate,
                "categoryName": '',
                "expertPic": expertprofilepic,
                "onlineStatus": 1,
                "DisplayName": expertdata.DisplayName,
                "freechatminute": 0,
                "expertCharge": expertdata.Charge,
                "expertUSDCharge": expertdata.Charge,
                "ReviewCount": 0,
                "Rating": 0,
                "ExpertUrl": expertUrl,
                "ClickOn": "CHAT",
                "expertCharges": expertChargeToShow,
                "currency": UserCurrency
            };
        }

        let clevertapEvntObj = {
            "Astrologer Name": expertdata.DisplayName,
            "Astrologer ID": expertdata.PsychicId,
            "Astrologer Rate": expertdata.ChargeRate,
            "Currency Rate": expertconfig.UserCurrency == 'INR' ? 1 : g_USD_INR_Rate,
            "Currency Code": expertconfig.UserCurrency,
            "Button Click": 1,
            "ExpertType": "Human",
            "Offer Type": localModuleOfferType
        };
        ...
        if (BaseRate > 0 && ((DiscountPer > 0 && !IsChargeOverride) || (BaseRate > expertChargeToShow && IsChargeOverride) || (BaseRate > expertChargeToShow)) && !IsFreeChat && localModuleOfferType != 6) {
            ...
            expertChargeBeforeDiscountStr = '<del class="text-lowercase">' + UserCurrency + '' + expertChargeBeforeDiscount + '</del> <span class="font-weight-bold color-red text-lowercase">' + UserCurrency + expertChargeToShow + getLanguageKeyString("EP_Charge_Min") + '</span>';
        } else {
            expertChargeBeforeDiscountStr = '<span class="font-weight-bold text-lowercase">' + UserCurrency + expertChargeToShow + getLanguageKeyString("EP_Charge_Min") + '</span>';
        }
        ...
        let _UBD = sessionStorage.getItem("_UBD");
        if (typeof _UBD === "string") {
            _UBD = JSON.parse(atob(_UBD));
            if (typeof _UBD.name === "string" && _UBD.name != '') {
                customerName = _UBD.name;
            }
        }

        let retHtml = '<div class="lpo-expert-pp-container-recommended px-2 d-flex align-items-center justify-content-between"><div class="lpo-expert-pp-left d-flex align-items-center"><div class="d-block text-center"><img src="' + expertprofilepic + '" class="img-fluid" /><p class="mb-0 text-nowrap font-12 font-weight-semi-bold">' + expertdata.DisplayName + '</p></div><p class="pl-2 pr-3 mb-0 fordesktop pl-3">Hey <span class="font-weight-bold">' + customerName + '</span>! Don\'t miss out - continue this chat for an exclusive price of just <span class="color-red">' + UserCurrency + expertChargeToShow + '/ min</span> and get the guidance you need right away! </p><p class="pl-2 mb-0 formobile pl-3">Hey ' + customerName + ' Continue this chat at a price of <span class="color-red">' + UserCurrency + expertChargeToShow + ' /min</span>  </p></div><div class="lpo-expert-pp-right"><a onclick=\'cleverTapEventAndRedirect("' + walletRechargeUrl + '",' + JSON.stringify(clevertapEvntObj) + ')\' class="btn btn-chat font-weight-bold px-5 d-flex align-items-center justify-content-center">Yes, Continue</a></div></div>';
        setPaymentSessionForAIWizard(JSON.stringify(AIWizardsessionDataobj));
        let retObj = {
            "html": retHtml,
            "walleturl": walletUrl
        }
    ...
    getPsychicCardRecommended(expertdata, expertconfig) {
        ...
        if (checkIfOfferAvailable(eEligibleArea.CHAT)) {
            ...
            if (localModuleOfferType > 0 && (localModuleOfferType == 5 || localModuleOfferType == 8 || localModuleOfferType == 9 || localModuleOfferType == 13)) {
                ...
                if (expertconfig.purchaseddeals != null && expertconfig.purchaseddeals.length > 0) {
                ...
                if (_userDealCount < 1) {
                    ...
                    if (isDiscountOfferFiltertoApply > 0) {
                        if (expertdata.doe > 0 && expertdata.showinfree > 0) {

                            let _OfferDiscountPer = _CurrentOfferObj.OfferDiscountPer;
                            let tempexpertChargeToShow = parseFloat(_OfferDiscountPer) / 100 * parseFloat(expertCharge);
                            let updatedDiscountedprice = expertCharge - tempexpertChargeToShow;

                            if (gIsIndia == "True") {
                                updatedDiscountedprice = Math.ceil(updatedDiscountedprice);

                            } else {
                                updatedDiscountedprice = parseFloat(updatedDiscountedprice).toFixed(2);
                            }
                            ...
                            if (gloggedInUserid > 0) {
                                if (parseInt(localmoduleofferId) > 0) {
                                    expertChargeToShow = updatedDiscountedprice;
                                    islowpriceoffer = true;
                                }
                            } else {
                                expertChargeToShow = updatedDiscountedprice;
            ...
            //else if (localModuleOfferType > 0 && (localModuleOfferType == 14)) { // discount offer
            //    lowPriceOffferMin = _CurrentOfferObj.OfferMinutes;
            //    let _userDealCount = 0;
            //    if (expertconfig.purchaseddeals != null && expertconfig.purchaseddeals.length > 0) {
            //        let expertDealsArray = expertconfig.purchaseddeals.filter(e => {
            ...
            //        if (expertCharge >= minExpertChargeRateRq) {
            //            if (expertdata.IsLowPrice || !isofferForLPOExpertsOnly) {
            //                let tempexpertChargeToShow = parseFloat(_OfferDiscountPer) / 100 * parseFloat(expertCharge);
            //                let updatedDiscountedprice = expertCharge - tempexpertChargeToShow;
        ...
        let chatclickUrl = expertUrl;
        if (chatclickUrl.indexOf("?") >= 0) {
            if (chatclickUrl.lastIndexOf("?") == chatclickUrl.length - 1) {
                chatclickUrl += "opaction=chat";
            } else if (chatclickUrl.indexOf("opaction") == -1) {
                chatclickUrl += "&opaction=chat";
            }
        } else {
            chatclickUrl += "?opaction=chat";
    ...
    getPsychicCard(expertdata, expertconfig) {        
        ...
        let happyHourTxt = "";
        let isHappyHourOfferAvailable = false;
        let happyhourEndTimestamp = 0;
        let updatedDiscountedprice = 0;
        ...
        if (checkIfOfferAvailable(eEligibleArea.CHAT)) {
            if (_CurrentOfferObj.OfferType > 0) {
                ...
                //            if (expertdata.IsLowPrice || !isofferForLPOExpertsOnly) {
                //                let tempexpertChargeToShow = parseFloat(_OfferDiscountPer) / 100 * parseFloat(expertCharge);
                //                 updatedDiscountedprice = expertCharge - tempexpertChargeToShow;
                ...
                //                    if (parseInt(localmoduleofferId) > 0) {
                //                        expertChargeToShow = updatedDiscountedprice;
                //                        //expertCharge = updatedDiscountedprice;
                //                        islowpriceoffer = true;
                //                    }
                //                } else {
                //                    expertChargeToShow = updatedDiscountedprice;
        ...
        if (islowpriceoffer || IsFreeChat) {
            expertdata.isHappyHourEnabled = 0;
            expertdata.HappyHourRate = 0;
            expertdata.HappyHourPercent = 0;
        }
        try {
            //happy hour work started here
            if (expertdata.isHappyHourEnabled == 1 && _isHappyHourAvailable && !(islowpriceoffer || IsFreeChat)) {
                isHappyHourOfferAvailable = true;

                expertCharge = parseFloat(expertdata.HappyHourRate);
                expertChargeToShow = parseFloat(expertdata.HappyHourRate);

                if (gIsIndia == "True") {
                    expertCharge = Math.ceil(expertCharge);
                    expertChargeToShow = Math.ceil(expertChargeToShow);
                } else {
                    expertCharge = parseFloat(expertCharge).toFixed(2);
                    expertChargeToShow = parseFloat(expertChargeToShow).toFixed(2);
                }


                 happyhourEndTimestamp = parseInt(expertdata.HappyHourEndTime.match(/\d+/)[0]);

                let ddf = new Date(happyhourEndTimestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });

                //var ddf = new Date(happyhourEndTimestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
                happyHourTxt = '<div class="expert-happy-hour mr-1" data-timestamp="' + happyhourEndTimestamp + '">' + getLanguageKeyString("GLB_HappyHourTxt").format(expertdata.HappyHourPercent, ddf, 0) + '</div>';
            }
            //happy hour ended here
        } catch (e) {
            console.log(e);
        }
        ...
        let expertstatus = expertdata.onlineStatus == 1 ? "Available" : (expertdata.onlineStatus == 2 ? "Busy" : "Unavailable");
        let expertstatusbadge = '<div id="psychic-' + expertdata.PsychicId + '-badge" class="status-badge specific-Clr-Busy" title="Busy"></div><div  class="status-badge-txt text-center specific-Clr-Busy"><span id="psychic-' + expertdata.PsychicId + '-badge-txt"title="' + getLanguageKeyString("CWA_Status_Busy") + '" class="status-badge-txt specific-Clr-Busy tooltipex">' + getLanguageKeyString("CWA_Status_Busy") +'</span></div>';
        if (expertdata.onlineStatus == 1) {
            expertstatusbadge = '<div id="psychic-' + expertdata.PsychicId + '-badge" class="status-badge specific-Clr-Online" title="Online"></div><div  class="status-badge-txt text-center specific-Clr-Online"><span id="psychic-' + expertdata.PsychicId + '-badge-txt" title="' + getLanguageKeyString("CWA_Status_Online") + '" class="status-badge-txt specific-Clr-Online tooltipex">' + getLanguageKeyString("CWA_Status_Online") +'</span></div>';
        } else if (expertdata.onlineStatus == 3) {
            expertstatusbadge = '<div id="psychic-' + expertdata.PsychicId + '-badge" class="status-badge specific-Clr-Offline" title="Offline"></div><div  class="status-badge-txt text-center specific-Clr-Offline"><span id="psychic-' + expertdata.PsychicId + '-badge-txt"title="' + getLanguageKeyString("CWA_Status_Offline") + '" class="status-badge-txt specific-Clr-Offline tooltipex">' + getLanguageKeyString("CWA_Status_Offline") + '</span></div>';
        }
        ...
        if (expertdata.hasdeal) {
            ...
            dealsection = '<div class="expert-deal-badge position-relative mr-1" style=" width:48px; height:15px;"><a href="javascript:void(0);" ' + usrActionTxt + '><span style="background:#5bbe2a; line-height:15px;" class="text-white rounded d-flex px-1 font-12 align-items-center pr-4">Deal <img src="' + _atacommonCdnPrefix +'dashaspeaks/web/content/images/deal-box.gif" width="14" height="16" class="position-absolute" style="top:-5px; right:4px; width:14px !important;">' + _userDealCountSection + '</span></a></div>';
        }
        ...
        let itemsection1 = '<ul class="list-unstyled d-flex mb-0"><li class = "mr-3 position-relative psychic-presence status-' + expertdata.PsychicId + '" data-status="' + expertstatus + '" data-chata="' + UserCurrency + '' + expertCharge + '" data-calla="' + UserCurrency + ' ' + expertCallCharge + '" ><a href="javascript:void(0);" data-href="' + expertUrl + '"><div class="psyich-img position-relative"><img src="' + expertprofilepic + '" width="80" height="80" style="border-radius:50%;" loading="lazy" /></div></a>' + expertstatusbadge + '</li ><li class="w-100 overflow-hidden"><a href="javascript:void(0);" data-href="' + expertUrl + '" class="colorblack font-weight-semi-bold font16 mt-0 ml-0 mr-0 mb-0 p-0 text-capitalize d-block" data-toggle="tooltip" title="' + expertdata.DisplayName + '">' + expertdata.DisplayName + '</a><span class="font-12 d-block color-red">' + categoryName + '</span><span class="font-12 d-block exp-language">' + ExpertLanguages + '</span><span class="font-12 d-block"> ' + getLanguageKeyString("EP_Exp") + ' : ' + expertExperience + ' ' + getLanguageKeyString("EP_ExpYear") + '</span><span class="font-12 font-weight-semi-bold d-flex"> <span class="exprt-price">' + expertChargeBeforeDiscountStr + '</span> <span class="free-badge text-uppercase font-weight-bold color-red ml-2 d-none">' + getLanguageKeyString("GLB_Free_Txt") + '</span></span></li></ul>';
        ...
        let itemsection2 = '<div class="d-flex align-items-center justify-content-between"><div class="psy-review-section" ><a href="javascript:void(0);" data-href="' + expertUrl + '"><p class="m-0 p-0 font9 nowrap review-psy">' + this.getReviews(expertdata.Rating, expertdata.ReviewCount) + '</p></a></div><div class="d-flex align-items-end position-relative"><div class="d-block">' + this.ExpertActionBtn(expertdata, {
            "CallChatBtnShow": CallChatBtnShow, "categoryName": CategoryId, "expertCharge": expertCharge, "expertUSDCharge": expertdata.Charge, "expertChargeToShow": expertChargeToShow, "UserCurrency": UserCurrency, "expertChargeType": expertChargeType, "expertlocaleurl": expertUrl, "isFreeChat": IsFreeChat, "IsChargeOverride": IsChargeOverride, "ModuleOfferType": ModuleOfferType, "NextTimeCount": NextTimeCount, "NextSessionCounter": NextSessionCounter, "moduleofferId": moduleofferId, "LowPriceOfferMin": lowPriceOffferMin, "IsHappyHour": isHappyHourOfferAvailable, "HappyHourEndTime": happyhourEndTimestamp, "updatedDiscountedprice": updatedDiscountedprice }) + '' + this.getFreeMinuteText(isrechargeofferavailable, FreeMinutesForEligibleOffer) +'</div></div></div>';


        let itemsection3 = '<div class="checkmark">' + dealsection +'</div >';

        return '<div class="psychic-card bg-white expertOnline ' + isofferapplicable + ' ' + (expertdata.Badge > 0 ? 'must-try-badge-applicable' : '') + '" data-FreeRank="' + expertdata.FreeRanking + '" data-LpoRank="' + expertdata.LpoRank + '" data-psychic-id="' + expertdata.PsychicId + '">' + happyHourTxt +'<div class="psychic-card-inner overflow-hidden position-relative">' + this.getMustTryBadge(expertdata.badgeid) + itemsection1 + itemsection2 + itemsection3 + '</div></div>';
    }
    ...
    checkIfExpertTakenRecentSession(dataitems) {
        ...
        if (_LastTimestampExpertsList != 'undefined' && _LastTimestampExpertsList != null && typeof _LastTimestampExpertsList === "object" && dataitems != null && _LastTimestampExpertsList.length > 0) {
