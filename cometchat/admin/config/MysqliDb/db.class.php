<?php
  ...
  class DB
  {
   ...
   header("location: install_step1.php?msg=$data");
    ...
    {
      return $this->queryUniqueValue("SELECT SUM(`$column`) FROM `$table` WHERE $where");
    }
	
	function sumOfJoin($column, $table, $where)
    {
      return $this->queryUniqueValue("SELECT SUM($column) FROM $table WHERE $where");
    }
	...
	function ratingClient($table, $where)
    {
			...
			if($star>0){
			return ceil($star/$rowcount);	
			}
			return 0;
    }
	function percentage($table, $where)
    {
			$query = "SELECT * FROM `$table` WHERE $where";
			$result = mysqli_query($this->connection, $query);
			$star=0;
			$rowcount=mysqli_num_rows($result);
			while($row=$result->fetch_assoc()) {	
				$star=$star+$row['recommend'];
			}
			if($star>0){
			return ($star/$rowcount)*100;	
			}
			return 0;
    }
  } // class DB
